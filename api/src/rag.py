...
class ConversationalRAGChain:
    ...
    def FAISS(self, data=None, add=False):
        vector_path = (r'C:\Users\gupta\OneDrive\Desktop\Projects\conversational_rag\api\FAISSdb\faiss_index')
        ...
        if not os.path.exists(vector_path):
            if data is not None:
                vector_store = FAISS.from_documents(documents=data, embedding=google_embed)
                vector_store.save_local(vector_path)
        else:
            vector_store = FAISS.load_local(vector_path, embeddings=google_embed, allow_dangerous_deserialization=True)
            if add==True and data is not None:
